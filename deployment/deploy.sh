#!/bin/bash
  ...
  mvn deploy --settings deployment/settings.xml -Dgpg.executable=gpg2 -Dgpg.keyname=DCF2BFEEB9C58B48 -Dgpg.passphrase=$PASSPHRASE -Dgpg.publicKeyring=$TRAVIS_BUILD_DIR/pubring.gpg -Dgpg.secretKeyring=$TRAVIS_BUILD_DIR/secring.gpg
  exit 0
#fi
